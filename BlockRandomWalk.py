...
def validCheck(cord:list,size:list,base_map:list[list]):
    if(cord[0]+size[0] >= len(base_map) or cord[1]+size[1] >= len(base_map)):
        return -1
...
def getCords(size:list,base_map:list[list]):
...
def Generate(room_amount:int,min_length:int,max_length:int):
    ...
    for room in room_data:
        room_cords = getCords(room,bitsMap)
